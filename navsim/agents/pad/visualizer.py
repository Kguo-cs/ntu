...
class PadCallback(pl.Callback):
    ...
    def _visualize_model(
        ...
        for sample_idx in range(self._num_rows * self._num_columns):
            all_plot = np.zeros((256, 256, 3), dtype=np.uint8)
            # plot[:224, :384] = (camera[sample_idx] * 255).astype(np.uint8)[::2, ::2]
            # plot[224:224*2, :384] = (camera[sample_idx] * 255).astype(np.uint8)[::2, ::2]
            # plot[224:224*3, :384] = (camera[sample_idx] * 255).astype(np.uint8)[::2, ::2]
            # plot[224:224*4, :384] = (camera[sample_idx] * 255).astype(np.uint8)[::2, ::2]
            gt_color, pred_color = (0, 255, 0), (255, 0, 0)

            plot1=plot_agent_on_semantic(bev[sample_idx], agent_states[sample_idx][agent_labels[sample_idx]],gt_color, self._config)
            plot2=plot_agent_on_semantic(pred_bev[sample_idx], pred_agent_states[sample_idx][pred_agent_labels[sample_idx] > 0.5],pred_color, self._config)
...
def plot_agent_on_semantic(bev,agent_state_array,color,config ):

    semantic_map=semantic_map_to_rgb(bev, config)
    #height, width = semantic_map.shape[:2]

    # def coords_to_pixel(coords):
    #     """Convert local coordinates to pixel indices."""
    #     pixel_center = np.array([[0, config.bev_pixel_width / 2.0]])
    ...
    #     cv2.polylines(semantic_map, [exterior], isClosed=True, color=color, thickness=2)
...
def dict_to_device(dict: Dict[str, torch.Tensor], device: Union[torch.device, str]) -> Dict[str, torch.Tensor]:
    ...
    for key in dict.keys():
        if type(dict[key]) is torch.Tensor:
