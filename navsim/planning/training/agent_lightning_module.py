...
class AgentLightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch: Tuple[Dict[str, Tensor], Dict[str, Tensor]], batch_idx: int):
        ...
        if 'Pad' in self.agent.name():
            ...
            trajectory=predictions["trajectory"][:,None]
            final_score,best_score,proposal_scores,l2,trajectoy_scores=self.agent.compute_score(targets,trajectory)
            mean_score=proposal_scores.mean()
            pdm_score=predictions["pdm_score"]
            target_pdm_score=pdm_score[torch.arange(len(pdm_score)), torch.argmax(pdm_score, dim=1)]
            score_error=torch.abs(target_pdm_score - proposal_scores).mean()
            logging_prefix="val"
